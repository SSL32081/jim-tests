...
jim_ifos = [H1, L1]
f_mins = [ifo.minimum_frequency for ifo in bilby_ifos]
f_maxs = [ifo.maximum_frequency for ifo in bilby_ifos]
...
for i in range(n_samples):
    ...
    # jim_params["t_c"] = sample["geocent_time"] - start_time
...
np.save("compare_GW150914_likelihoods_NEW.npy", diff_logLs)
...
duration = jim_ifos[0].data.n_time * jim_ifos[0].data.delta_t
ax.set_title(f'Current inferred duration = {duration:.6f} s')
...
duration = (jim_ifos[0].data.n_time - 1) * jim_ifos[0].data.delta_t
ax.set_title(f'Newly inferred duration = {duration:.6f} s')
# ax.set_title(f'Newly inferred duration = {strain_duration:.6f} s')
ax.set_ylabel(r'$ \ln {\cal L}_{\rm Bilby} - \ln {\cal L}_{\rm Jim}$')
...
fig.savefig('compare_GW150914_time_likelihoods_NEW.pdf')